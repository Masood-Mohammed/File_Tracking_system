...
def migrate():
    ...
    if not os.path.exists(DB_NAME):
        ...
        # For simplicity, let's assume running from `backend/` where `instance/` is a child.
        print(f"Database not found at {DB_NAME}. Checking current directory...")
        if os.path.exists('file_tracking.db'):
            DB_NAME = 'file_tracking.db'
        else:
